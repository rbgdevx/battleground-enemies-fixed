  ...
  if mode == "Import" then
    ...
    importButton:SetCallback("OnClick", function(widget, ...)
      if frame:GetUserData("mode") == "Import" then
        ...
        if error then
          return BattleGroundEnemies:Information(error)
        end
...
function BattleGroundEnemies:ExportDataCompressed(data, forPrint)
  ...
  if not serialized then
    return false, "An serialization error happened"
  end

  local compressed = LibDeflate:CompressDeflate(serialized)
  if not compressed then
    return false, "An compression error happened"
  end
...
function BattleGroundEnemies:DecodeReceivedData(encoded, fromPrint)
  ...
  if not decoded then
    return false, "An decoding error happened"
  end
  local decompressed = LibDeflate:DecompressDeflate(decoded)
  if not decompressed then
    return false, "An decompressing error happened"
  end
