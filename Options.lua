...
local function isValidPlayerCountRange(playerCountConfigs, isCustom, inputs, profileToIgnore)
  ...
  if not min then
    return false, L.MinCantBeUndefined
  end
  if not max then
    return false, L.MaxCantBeUndefined
  end

  if min > max then
    return false, L.MaxNeedsToBeEqualOrGreater
  end
  ...
  for i = 1, #playerCountConfigs do
    ...
    if not profileToIgnore or playerCountConfig ~= profileToIgnore then
      ...
      if Data.Helpers.AreOverlappingRanges(newRange, range) then
        return false,
          L.RangeOverlapping:format(BattleGroundEnemies:GetPlayerCountConfigNameLocalized(playerCountConfig, isCustom))
      end
    end
  end
  return true
...
function BattleGroundEnemies:ModuleFrameNeedsHeight(moduleFrame, config)
  ...
  if flags.HasDynamicSize then
    return false
  end

  local heightFlag = flags.Width

  if heightFlag == "Fixed" then
    return
  end

  local activePoints = self:GetActivePoints(config)
  if not activePoints then
    return
  end
...
function BattleGroundEnemies:ModuleFrameNeedsWidth(moduleFrame, config)
  if not moduleFrame or not moduleFrame.flags then
    return false
  end
  local flags = moduleFrame.flags
    ...
    for playerName, playerButton in pairs(players) do
      ...
      if isDependant then
        ...
        BattleGroundEnemies:Information(
          "You can't anchor this module's frame to this frame because this would result in looped frame anchoring because the frame or one of the frame that this frame is dependant on are already attached to this module."
        )
        return false
      else
        return true
...
function Data.AddPositionSetting(location, moduleName, moduleFrame, playerType)
  moduleFrame = moduleFrame
    or BattleGroundEnemies.ButtonModules[moduleName]
    or BattleGroundEnemies.ButtonModules[moduleName:lower()]
  ...
  temp.WidthGroup = {
    ...
    args = {
      ...
      Width = {
        ...
        step = 1,
        hidden = function()
          local hidden = location.UseButtonHeightAsWidth or location.UseButtonWidthAsWidth
          if hidden then
            BattleGroundEnemies:NotifyChange()
            return true
          end
...
function Data.AddContainerSettings(location)
  return {
    IconSize = {
      ...
      order = 1,
      hidden = function()
        return location.UseButtonHeightAsSize
      end,
    },
      ...
      temp[moduleName] = {
        ...
        args = {
          ...
          Reset = {
            type = "execute",
            name = SETTINGS_DEFAULTS,
            desc = L.ResetModule_Desc:format(
              L[playerType],
              BattleGroundEnemies:GetPlayerCountConfigNameLocalized(location)
            ),
            func = function()
...
local function addEnemyAndAllySettings(self, mainFrame)
  ...
  settings.GeneralSettings = {
    ...
    args = {
      ...
      CopySettings = {
        ...
        set = function(option, value)
          if value == "Normal" then
            BattleGroundEnemies.db.profile[playerType] =
              CopyTable(BattleGroundEnemies.db.profile[oppositePlayerType], false)
          elseif value == "Mirrored" then
            BattleGroundEnemies.db.profile[playerType] =
              BattleGroundEnemies:FlipSettingsHorizontallyRecursive(BattleGroundEnemies.db.profile[oppositePlayerType])
          end
          BattleGroundEnemies:NotifyChange()
        end,
        values = {
      ...
      LoadDefaults = {
        ...
        func = function()
          BattleGroundEnemies.db.profile[playerType] =
            CopyTable(BattleGroundEnemies.db.defaults.profile[playerType], false)
          BattleGroundEnemies:NotifyChange()
        end,
        confirm = function()
          return "Are you sure?"
      ...
      RangeIndicator_Settings = {
        ...
        args = {
          ...
          RangeIndicator_Frames = {
            ...
            hidden = function()
              return (not location.RangeIndicator_Enabled or location.RangeIndicator_Everything)
            end,
          ...
          LeftButton = {
            ...
            args = {
              ...
              LeftButtonValue = {
                ...
                disabled = function()
                  return location.LeftButtonType == "Target" or location.LeftButtonType == "Focus"
                end,
                multiline = true,
                width = "double",
                order = 2,
          ...
          RightButton = {
            ...
            args = {
              ...
              RightButtonValue = {
                ...
                desc = L.CustomMacro_Desc,
                disabled = function()
                  return location.RightButtonType == "Target" or location.RightButtonType == "Focus"
                end,
                multiline = true,
                width = "double",
                order = 2,
          ...
          MiddleButton = {
            ...
            args = {
              ...
              MiddleButtonValue = {
                ...
                desc = L.CustomMacro_Desc,
                disabled = function()
                  return location.MiddleButtonType == "Target" or location.MiddleButtonType == "Focus"
                end,
                multiline = true,
                width = "double",
                order = 2,
  ...
  for i = 1, #playerCountConfigs do
    ...
    table.insert(
      allPlayerCountConfigOptionsNames,
      L[playerType] .. ": " .. BattleGroundEnemies:GetPlayerCountConfigNameLocalized(playerCountConfigs[i], false)
    )
  end
  ...
  if BattleGroundEnemies.db.profile[playerType].CustomPlayerCountConfigsEnabled then
    ...
    for i = 1, #customPlayerCountConfigs do
      ...
      table.insert(
        ...
        L[playerType]
          .. ": "
          .. BattleGroundEnemies:GetPlayerCountConfigNameLocalized(customPlayerCountConfigs[i], true)
      )
    end
  else
  ...
  for i = 1, #allDbLocations do
    ...
    local currentProfileName =
      BattleGroundEnemies:GetPlayerCountConfigNameLocalized(playerCountLocation, isCustomProfile)
    ...
    settings[BattleGroundEnemies:GetPlayerCountConfigName(playerCountLocation)] = {
      ...
      args = {
        ...
        CopySettings = {
          ...
          set = function(option, value)
            overWriteSettigsKeepPlayerCount(
              thisPlayerCountConfigs,
              indexThisPlayerCountConfg,
              allConfigsWithoutCurrent[value]
            )
          end,
          values = allPlayerCountConfigOptionsNamesWithoutCurrent,
          confirm = function()
            return L.ConfirmProfileOverride:format(
              L[playerType] .. ": " .. currentProfileName,
              L[playerType] .. ": " .. currentProfileName
            )
          end,
          width = 1.5,
        ...
        LoadFromDefaultPlayerCountProfile = {
          ...
          set = function(option, value)
            overWriteSettigsKeepPlayerCount(
              thisPlayerCountConfigs,
              indexThisPlayerCountConfg,
              isCustomProfileEnabled and playerCountConfigDefaults[value] or playerCountConfigDefault
            )
          end,
          values = function()
            local t = {}
            for k = 1, #playerCountConfigDefaults do
              table.insert(
                ...
                L[playerType]
                  .. ": "
                  .. BattleGroundEnemies:GetPlayerCountConfigNameLocalized(playerCountConfigDefaults[k], false)
              )
            end
            return t
          end,
        ...
        blub = {
          ...
          args = {
            ...
            ChangePlayerCount = {
              ...
              confirm = function()
                return "Are you sure you want to change this profile so its used for "
                  .. tempInputs.MinPlayerCount
                  .. " to "
                  .. tempInputs.MaxPlayerCount
                  .. "players for "
                  .. playerType
                  .. " ?"
              end,
            ...
            errormsg = {
              ...
              fontSize = "large",
              name = function()
                local isGood, errorMessage =
                  isValidPlayerCountRange(thisPlayerCountConfigs, true, tempInputs, playerCountLocation)
                if not isGood then
                  return ERRORS .. ": " .. errorMessage
                end
              end,
              hidden = function()
                if not isCustomProfileEnabled then
                  return true
                end
        ...
        MainFrameSettings = {
          ...
          disabled = function()
            return not playerCountLocation.Enabled
          end,
          --childGroups = "tab",
          order = 6,
          args = {
        ...
        ButtonSettings = {
          ...
          args = {
            ...
            BarHorizontalGrowdirection = {
              ...
              hidden = function()
                return playerCountLocation.BarColumns < 2
              end,
              disabled = InCombatLockdown,
              values = Data.HorizontalDirections,
              order = 7,
        ...
        ModuleSettings = {
          ...
          name = L.Modules,
          order = 8,
          args = self:AddModulesSettings(playerCountLocation, playerCountConfigDefault, playerType, function(options)
            return not options.attachSettingsToButton
          end),
        },
      },
    }
    Mixin(
      settings[BattleGroundEnemies:GetPlayerCountConfigName(playerCountLocation)].args.ButtonSettings.args,
      self:AddModulesSettings(playerCountLocation, playerCountConfigDefault, playerType, function(options)
        return options.attachSettingsToButton
      end)
    )
  end
      ...
      TestmodeSettings = {
        ...
        args = {
          ...
          Testmode_ToggleAnimation = {
            ...
            disabled = function()
              return InCombatLockdown() or not self.states.testmodeActive
            end,
      ...
      GeneralSettings = {
        ...
        args = {
          ...
          miscellaneous = {
            ...
            args = {
              Reset = {
                type = "execute",
                name = SETTINGS_DEFAULTS,
                func = addResetFunctionForgroup(
                  BattleGroundEnemies.db.profile,
                  BattleGroundEnemies.db.defaults.profile
                ),
                width = "full",
                order = 1,
              },
          ...
          PlayerCount = {
            ...
            order = 8,
            args = Data.AddNormalTextSettings(
              location.PlayerCount.Text,
              BattleGroundEnemies.db.defaults.profile.PlayerCount.Text
            ),
          },
          ButtonModules = {
            type = "group",
...
-- DEBUG (TEMP): auto-dump every objective carrier's LIVE secure-click state on
-- token changes AND on each target change (the moment a click can break it).
-- ARENA shows the binding at acquisition; TARGET shows it right after a click.
-- Only carriers (a bound arena token) log, so it's quiet when none are held.
-- Logs SILENTLY to the SavedVariables jitter log (no chat prints) — read off
-- disk after a /reload like the health-jitter entries. Remove once the
-- Deephaul/EotS carrier-click fix is confirmed in-game.
local function bgeDumpCarriers(reason)
  local function dumpSide(side, label)
    if not (side and side.Players) then
      return
    end
    for _, btn in pairs(side.Players) do
      local pad = btn.PlayerDetails
  ...
  dumpSide(BattleGroundEnemies.Enemies, "E")
  dumpSide(BattleGroundEnemies.Allies, "A")
end

local bgeCarrierDbg = CreateFrame("Frame")
bgeCarrierDbg:RegisterEvent("ARENA_OPPONENT_UPDATE")
bgeCarrierDbg:RegisterEvent("PLAYER_TARGET_CHANGED")
bgeCarrierDbg:SetScript("OnEvent", function(_, event)
  if not BattleGroundEnemies:IsInPvPInstance() then
    return
  end
  bgeDumpCarriers(event == "ARENA_OPPONENT_UPDATE" and "arena" or "click")
end)

-- Also dump every 2s while a carrier is held. A click that does NOTHING (even the
-- FIRST one) fires no PLAYER_TARGET_CHANGED, so the event hooks alone can't catch
-- a never-clickable carrier. This keeps the live binding visible continuously
-- (silent when no carrier is held). Remove with the rest of the debug.
C_Timer.NewTicker(2, function()
  if BattleGroundEnemies:IsInPvPInstance() then
    bgeDumpCarriers("tick")
  end
end)
