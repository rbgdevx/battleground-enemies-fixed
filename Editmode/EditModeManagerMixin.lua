...
function BattleGroundEnemies.Mixins.CustomEditModeManagerFrameMixin:RegisterSystemFrame(systemFrame)
  for i = 1, #self.registeredSystemFrames do
    if self.registeredSystemFrames[i] == systemFrame then
      return
    end
  end
...
function BattleGroundEnemies.Mixins.CustomEditModeManagerFrameMixin:UpdateSystemAnchorInfo(systemFrame)
  ...
  local systemInfo = {
    anchorInfo = ConvertToAnchorInfo(
      firstPoint.Point,
      firstPoint.RelativeFrame,
      firstPoint.RelativePoint,
      firstPoint.OffsetX or 0,
      firstPoint.OffsetY or 0
    ),
  } --self:GetActiveLayoutSystemInfo(systemFrame.system, systemFrame.systemIndex);
  if systemInfo then
    ...
    if relativeTo and relativeTo.GetName then
      BattleGroundEnemies:Debug(
        ...
        relativeTo.playerButton
          and relativeTo.playerButton.PlayerDetails
          and relativeTo.playerButton.PlayerDetails.PlayerName
      )
    end
    ...
    if not relativeTo then
      ...
      local scaleSystemFrame = tonumber(systemFrame:GetEffectiveScale()) or 1
      local originalSystemFrameLeft = (tonumber(systemFrame:GetLeft()) or 0) * scaleSystemFrame
      local originalSystemFrameTop = (tonumber(systemFrame:GetTop()) or 0) * scaleSystemFrame
      --BattleGroundEnemies:Debug("3", originalSystemFrameLeft, originalSystemFrameTop)

      local scaleRelativeTo = (relativeTo and relativeTo.GetEffectiveScale and tonumber(relativeTo:GetEffectiveScale()))
        or 1
      local relativeLeft = (relativeTo and relativeTo.GetLeft and tonumber(relativeTo:GetLeft()) or 0) * scaleRelativeTo
      local relativeTop = (relativeTo and relativeTo.GetTop and tonumber(relativeTo:GetTop()) or 0) * scaleRelativeTo
      ...
      offsetX = (originalSystemFrameLeft - relativeLeft) / scaleSystemFrame
      offsetY = (originalSystemFrameTop - relativeTop) / scaleSystemFrame
    ...
    -- Apply scale adjustment: PlayerButton.lua divides by effectiveScale when applying,
    -- so we need to multiply by effectiveScale when saving to compensate
    local frameScale = systemFrame:GetEffectiveScale() or 1
...
function BattleGroundEnemies.Mixins.CustomEditModeManagerFrameMixin:OnSystemSettingChange(
  systemFrame,
  changedSetting,
  newValue
)
...
function BattleGroundEnemies.Mixins.CustomEditModeManagerFrameMixin:SelectSystem(selectFrame)
  ...
  local function selectMatchingSystem(index, systemFrame)
    if
      systemFrame.system == selectFrame.system
      and systemFrame.playerButton.PlayerType == selectFrame.playerButton.PlayerType
    then
...
function BattleGroundEnemies.Mixins.CustomEditModeManagerFrameMixin:RefreshSnapPreviewLines()
  ...
  if not self.magnetismPreviewLinesPool then
    self.magnetismPreviewLinePool =
      EditModeUtil.CreateLinePool(self.MagnetismPreviewLinesContainer, "MagnetismPreviewLineTemplate")
  end

  local magneticFrameInfos =
    BattleGroundEnemies.CustomEditModeMagnetismManager:GetMagneticFrameInfos(self.snapPreviewFrame)
...
function BattleGroundEnemies.Mixins.CustomEditModeManagerFrameMixin:SetEnableAdvancedOptions(
