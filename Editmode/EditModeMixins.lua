...
EditModeSystemSettingsDialog.UpdateDialog = function(self, systemFrame) end
...
local EditModeSystemSelectionLayout = EditModeSystemSelectionLayout
  or {
    ...
    ["setupPieceVisualsFunction"] = not (C_Texture and C_Texture.GetAtlasInfo(
      "editmode-actionbar-selected-nineslice-corner"
    )) and function(container, piece, setup, pieceLayout, textureKit, userLayout)
      ...
      piece:SetTexture([[Interface\AddOns\BattleGroundEnemiesFixed\Textures\Editmode\]] .. suffixPath)
...
function BattleGroundEnemies.Mixins.EditModeSystemSelectionBaseMixin:OnLoad()
  ...
  if self.HorizontalLabel then
    self.HorizontalLabel:SetFontObjectsToTry(
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:OnSystemLoad()
  ...
  if not self.isSystemLoaded then
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:PrepareForSave() end
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:SetPointOverride(
  point,
  relativeTo,
  relativePoint,
  offsetX,
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:TrySetCompositeNumberSettingValue(setting, newValue)
  ...
  if rawOldValue ~= rawNewValue then
    ...
    BattleGroundEnemies.EditMode.EditModeManager:OnSystemSettingChange(
      self,
      settingDisplayInfo.compositeNumberHundredsSetting,
      hundredsValue
    )
    ...
    BattleGroundEnemies.EditMode.EditModeManager:OnSystemSettingChange(
      self,
      settingDisplayInfo.compositeNumberTensAndOnesSetting,
      tensAndOnesValue
    )
  end
  return true
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:UpdateSystemSetting(setting, entireSystemUpdate)
  ...
  if self:IsSettingDirty(setting) then
    BattleGroundEnemies.EditMode.EditModeManager:MirrorSetting(
      self.system,
      self.systemIndex,
      setting,
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:GetScaledSelectionCenter()
  ...
  if self.Selection then
    local cX, cY = self.Selection:GetCenter()
    centerX = cX or 0
    centerY = cY or 0
  end
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:GetScaledCenter()
  ...
  local centerX = cX or 0
  local centerY = cY or 0
  local scale = self:GetScale() or 1
  return centerX * scale, centerY * scale
end
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:GetScaledSelectionSides()
  ...
  if self.Selection then
    local sLeft, sBottom, sWidth, sHeight = self.Selection:GetRect()
    left = tonumber(sLeft) or 0
    bottom = tonumber(sBottom) or 0
    width = tonumber(sWidth) or 0
    height = tonumber(sHeight) or 0
  end
  local scale = tonumber(self:GetScale()) or 1
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:BreakFrameSnap(deltaX, deltaY)
  ...
  if top then
    local scale = math.max(0.01, self:GetScale() or 1)
    local offsetY = -((UIParent:GetHeight() - top * scale) / scale)

    local offsetX, anchorPoint
    if self.alwaysUseTopRightAnchor then
      offsetX = -((UIParent:GetWidth() - (self:GetRight() or 0) * scale) / scale)
      anchorPoint = "TOPRIGHT"
    else
      offsetX = self:GetLeft() or 0
      anchorPoint = "TOPLEFT"
    end
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:GetFrameMagneticEligibility(systemFrame)
  ...
  local horizontalEligible = self:IsVerticallyAlignedWithFrame(systemFrame)
    and (self:IsToTheLeftOfFrame(systemFrame) or self:IsToTheRightOfFrame(systemFrame))
  local verticalEligible = self:IsHorizontallyAlignedWithFrame(systemFrame)
    and (self:IsAboveFrame(systemFrame) or self:IsBelowFrame(systemFrame))
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:OnEditModeEnter()
  if not self.defaultHideSelection and self.Enabled then
...
function BattleGroundEnemies.Mixins.CustomEditModeSystemMixin:OnAnyEditModeSystemAnchorChanged() end
