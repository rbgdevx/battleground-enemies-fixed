  ...
  function EditModeUtil:GetSettingMapFromSettings(settings, displayInfoMap)
    ...
    for _, settingInfo in ipairs(settings) do
      ...
      if displayInfoMap and displayInfoMap[settingInfo.setting] then
        settingMap[settingInfo.setting].displayValue =
          displayInfoMap[settingInfo.setting]:ConvertValueForDisplay(settingInfo.value)
      end
    end
    return settingMap
  end
...
local CustomEditModeMagnetismManager = {}

hooksecurefunc("UpdateUIParentPosition", function()
  CustomEditModeMagnetismManager:UpdateUIParentPoints()
end)

CustomEditModeMagnetismManager.magneticFrames = {}

-- Default magnetism range
CustomEditModeMagnetismManager.magnetismRange = 8
CustomEditModeMagnetismManager.sqrMagnetismRange = CustomEditModeMagnetismManager.magnetismRange
  * CustomEditModeMagnetismManager.magnetismRange
CustomEditModeMagnetismManager.sqrCornerMagnetismRange = CustomEditModeMagnetismManager.sqrMagnetismRange * 2

function CustomEditModeMagnetismManager:UpdateUIParentPoints()
...
function CustomEditModeMagnetismManager:SetMagnetismRange(magnetismRange)
...
function CustomEditModeMagnetismManager:RegisterFrame(frame)
  self.magneticFrames[frame] = true
end

function CustomEditModeMagnetismManager:UnregisterFrame(frame)
  self.magneticFrames[frame] = nil
end

function CustomEditModeMagnetismManager:RegisterGrid()
  self.magneticGridLines = { horizontal = {}, vertical = {} }
end

function CustomEditModeMagnetismManager:UnregisterGrid()
  self.magneticGridLines = nil
end

function CustomEditModeMagnetismManager:RegisterGridLine(line, verticalLine, centerOffset)
...
function CustomEditModeMagnetismManager:GetMagneticFrameInfoTable(
  frame,
  point,
  relativePoint,
  distance,
  ...
  return {
    ...
    isCornerSnap = isCornerSnap,
  }
end

function CustomEditModeMagnetismManager:CheckReplaceMagneticFrameInfo(
  currentMagneticFrameInfo,
  frame,
  point,
  relativePoint,
  distance,
...
function CustomEditModeMagnetismManager:GetUIParentCheckPoints(systemFrame, verticalLines)
...
function CustomEditModeMagnetismManager:GetGridLineCheckPoints(systemFrame, verticalLines)
  local systemFrameCenterX, systemFrameCenterY = systemFrame:GetScaledSelectionCenter()
  local systemFrameLeft, systemFrameRight, systemFrameBottom, systemFrameTop = systemFrame:GetScaledSelectionSides()
...
function CustomEditModeMagnetismManager:FindClosestGridLine(systemFrame, verticalLines)
...
function CustomEditModeMagnetismManager:IsPotentialMagneticCornerFrame(frame)
  return frame and frame.GetScaledSelectionSides
end

function CustomEditModeMagnetismManager:ShouldReplaceClosestCorner(closestSqrDistance, cornerSqrDistance)
  return cornerSqrDistance <= self.sqrCornerMagnetismRange
    and (not closestSqrDistance or cornerSqrDistance < closestSqrDistance)
end
...
function CustomEditModeMagnetismManager:GetCornerMagneticFrameInfo(frame, relativeToFrameInfo)
  if
    not self:IsPotentialMagneticCornerFrame(frame)
    or not relativeToFrameInfo
    or not self:IsPotentialMagneticCornerFrame(relativeToFrameInfo.frame)
  then
    return nil
  end
  ...
  local relativeToFrameLeft, relativeToFrameRight, relativeToFrameBottom, relativeToFrameTop =
    relativeToFrameInfo.frame:GetScaledSelectionSides()
  ...
  for framePointName, framePointPosition in pairs(framePoints) do
    for relativeToFramePointName, relativeToFramePointPosition in pairs(relativeToFramePoints) do
      ...
      if
        not (
          (framePointName == "TOPLEFT" and relativeToFramePointName == "BOTTOMRIGHT")
          ...
          or (framePointName == "BOTTOMRIGHT" and relativeToFramePointName == "TOPLEFT")
        )
      then
        -- Check if this corner is closer
        cornerSqrDistance = CalculateDistanceSq(
          framePointPosition.x,
          framePointPosition.y,
          relativeToFramePointPosition.x,
          relativeToFramePointPosition.y
        )
        if self:ShouldReplaceClosestCorner(closestSqrDistance, cornerSqrDistance) then
          closestPoint, closestRelativePoint, closestSqrDistance =
            framePointName, relativeToFramePointName, cornerSqrDistance
        end
      end
    end
  end
  ...
  if closestSqrDistance then
    ...
    return self:GetMagneticFrameInfoTable(
      relativeToFrameInfo.frame,
      closestPoint,
      closestRelativePoint,
      math.sqrt(closestSqrDistance),
      offset,
      relativeToFrameInfo.isHorizontal,
      isCornerSnap
    )
  end
...
function CustomEditModeMagnetismManager:GetMagneticFrameInfoOptions(systemFrame)
  ...
  for _, frame in ipairs(eligibleFrames.horizontal) do
    ...
    horizontalMagneticFrameInfo = self:CheckReplaceMagneticFrameInfo(
    ...
    if self:IsPotentialMagneticCornerFrame(frame) then
      potentialHorizontalCornerMagneticFrame = self:CheckReplaceMagneticFrameInfo(
  ...
  for _, frame in ipairs(eligibleFrames.vertical) do
    ...
    verticalMagneticFrameInfo = self:CheckReplaceMagneticFrameInfo(
    ...
    if self:IsPotentialMagneticCornerFrame(frame) then
      potentialVerticalCornerMagneticFrame = self:CheckReplaceMagneticFrameInfo(
  ...
  local horizontalCornerMagneticFrameInfo =
    self:GetCornerMagneticFrameInfo(systemFrame, potentialHorizontalCornerMagneticFrame)
  local verticalCornerMagneticFrameInfo =
    self:GetCornerMagneticFrameInfo(systemFrame, potentialVerticalCornerMagneticFrame)
  local cornerMagneticFrameInfo
  if
    horizontalCornerMagneticFrameInfo
    and (
      not verticalCornerMagneticFrameInfo
      or horizontalCornerMagneticFrameInfo.distance < verticalCornerMagneticFrameInfo.distance
    )
  then
...
function CustomEditModeMagnetismManager:GetMagneticFrameInfos(systemFrame)
  local horizontalMagneticFrameInfo, verticalMagneticFrameInfo, cornerMagneticFrameInfo =
    self:GetMagneticFrameInfoOptions(systemFrame)
  ...
  elseif horizontalMagneticFrameInfo or verticalMagneticFrameInfo then
    ...
    if
      horizontalMagneticFrameInfo
      and (not verticalMagneticFrameInfo or horizontalMagneticFrameInfo.distance < verticalMagneticFrameInfo.distance)
    then
...
function CustomEditModeMagnetismManager:GetPreviewLineAnchors(magneticFrameInfo)
...
BattleGroundEnemies.CustomEditModeMagnetismManager = CustomEditModeMagnetismManager
